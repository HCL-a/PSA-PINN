...
for epoch in range(lbfgs_epochs):
    if (epoch + 1) % updata_interval == 0:
        ...
        l4 = l4.squeeze()
        l5 = l5.squeeze()
        _, mask1 = torch.topk(l1, int(select(q, epoch + 1, 25000) * len(l1)), largest=False)
        _, mask2 = torch.topk(l2, int(select(q, epoch + 1, 25000) * len(l2)), largest=False)
        _, mask3 = torch.topk(l4, int(select(q, epoch + 1, 25000) * len(l4)), largest=False)
        _, mask4 = torch.topk(l5, int(select(q, epoch + 1, 25000) * len(l5)), largest=False)
        # _, mask1 = torch.topk(l1, int(q * len(l1)), largest=False)
        # _, mask2 = torch.topk(l2, int(q * len(l2)), largest=False)
        # _, mask3 = torch.topk(l4, int(q * len(l4)), largest=False)
        # _, mask4 = torch.topk(l5, int(q * len(l5)), largest=False)
